...
namespace YiZi::Client
{
    ...
    User::User(const uint32_t id, CStringA phone, CString nickname, const uint32_t join_time, const bool is_admin)
        : m_Id{id}, m_Phone{std::move(phone)}, m_Nickname{std::move(nickname)}, m_JoinTime{join_time}, m_IsAdmin{is_admin} {}

    User* User::New(const uint32_t id, CStringA phone, CString nickname, const uint32_t join_time, const bool is_admin)
    {
        Delete();
